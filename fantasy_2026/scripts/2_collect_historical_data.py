...
def collect_historical_data_for_players(roster_df, existing_logs, seasons, rate_limit_ms=600):
    ...
    for i, player_name in enumerate(player_names, 1):
        ...
        # IMPORTANT: Always re-fetch current season to get latest games
        ...
        if player_normalized in existing_players_normalized:
            ...
            if player_normalized in existing_player_seasons:
                ...
                # ALWAYS include current season to get latest games
                seasons_to_fetch = [s for s in seasons if s not in existing_seasons or s == current_season]

                if not seasons_to_fetch:
                    print(f"  ✓ Already have all {len(existing_seasons)} seasons, skipping")
                    continue

                if current_season in seasons_to_fetch and current_season in existing_seasons:
                    print(f"  → Have {len(existing_seasons)} seasons, re-fetching {current_season} for latest games")
                else:
...
def save_historical_data(new_data, existing_data, output_dir):
    ...
    if not existing_data.empty and not new_data.empty:
        ...
        # Note: Column is 'Game_ID' (from NBA API), not 'GAME_ID'
        game_id_col = 'Game_ID' if 'Game_ID' in combined.columns else 'GAME_ID'
        if game_id_col in combined.columns and 'PLAYER_NAME' in combined.columns:
            before_dedup = len(combined)
            combined = combined.drop_duplicates(subset=['PLAYER_NAME', game_id_col], keep='last')  # keep='last' to prefer newer data
            after_dedup = len(combined)
            if before_dedup != after_dedup:
                print(f"  (Removed {before_dedup - after_dedup} duplicate game records)")
